...
export default function CheckoutModal({ open, onClose }) {
  ...
  function validate() {
    const e = {};
    if (!form.customerName.trim() || form.customerName.trim().length < 2) e.customerName = "Name must be at least 2 characters";
  ...
  return (
    <>
      <div className="fixed inset-0 bg-ink/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
        <div className="bg-cream w-full max-w-md rounded-2xl shadow-2xl animate-slide-up overflow-hidden" onClick={(e) => e.stopPropagation()}>
          {/* Header */}
          <div className="px-6 py-5 border-b border-brand-100 flex items-center justify-between">
            <h2 className="text-xl">Checkout</h2>
            <button onClick={onClose} className="text-ink/40 hover:text-ink transition-colors text-2xl" aria-label="close checkout modal">
              ×
            </button>
          </div>
